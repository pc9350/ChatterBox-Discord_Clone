...
const getDirectMessage = async (
  ...
  if (!dm) throw new Error("Direct message does not exist");
  const otherMember = await ctx.db
    .query("directMessageMembers")
    .withIndex("by_direct_message", (q) => q.eq("directMessage", id))
    .filter((q) => q.neq(q.field("user"), ctx.user._id))
    .first();
  if (!otherMember) throw new Error("Direct message has no other members");
  const user = await ctx.db.get(otherMember.user);
  if (!user) throw new Error("Other member does not exist.");
  return { ...dm, user };
};
